...
exports.artist_delete_get = asyncHandler(async (req, res, next) => {
    const [artist, artist_albums] = await Promise.all([
...
exports.artist_update_get = asyncHandler(async (req, res, next) => {
    const [artist, allGenres] = await Promise.all([
        Artist.findById(req.params.id).exec(),
        Genre.find({}).exec(),
    ]);
    ...
    for (const genre of allGenres) {
        for (const artist_genre of artist.genres) {
            if (genre._id.toString() === artist_genre._id.toString()) {
                genre.checked = "true";
            }
        }
    }
